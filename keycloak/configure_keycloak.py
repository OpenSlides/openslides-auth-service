...
def create_or_get_user(keycloak_admin, realm_name, users: list[tuple[str, int, str]]):
    keycloak_admin.connection.realm_name = realm_name
...
def configure_keycloak(keycloak_admin):
    global realm_name
    realm_name = "os"
    ...
    # Execute functions
    create_or_get_realm(keycloak_admin, realm_name)
    create_or_get_client_scope(keycloak_admin, realm_name, client_scope_name, protocol_mappers)
    print(f"Configuring Keycloak... in realm {realm_name}")
    create_or_get_client(keycloak_admin, realm_name, client_name, client_scope_name)
    clients = keycloak_admin.get_clients()
    client_scopes = keycloak_admin.get_client_scopes()
    ...
    configure_authenticator(keycloak_admin, realm_name)
...
def configure_authenticator(keycloak_admin: KeycloakAdmin, realm_name: str):
    # Schritt 1: Login-Flow duplizieren
    keycloak_admin.connection.realm_name = realm_name
    flow_alias = "openslides-browser-flow"
    # get flow with name "browser"  (AuthenticationFlowRepresentation)
    existing_flow = next(filter(lambda flow: flow.get('alias') == flow_alias,
                                list(keycloak_admin.get_authentication_flows())), None)

    if not existing_flow:
        keycloak_admin.copy_authentication_flow({"newName": flow_alias,}, "browser")

    executions = keycloak_admin.get_authentication_flow_executions(flow_alias)
    max_priority = max(execution["priority"] for execution in executions) if executions else 0

    authenticator_id = keycloak_admin.create_authentication_flow_execution({
        "provider": "openslides-authenticator",
    ...
    keycloak_admin.update_realm("os", {
        "realm": "os",
